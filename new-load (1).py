...
def watch_for_inserts():
    print("Watching for new documents...")
    with collection.watch([{'$match': {'operationType': 'insert'}}]) as stream:
        for change in stream:
            new_doc = change['fullDocument']
            if "embedding" in new_doc:
                print("Already embedded")
                continue  # Already embedded (just in case)
            text = serialize_product_for_embedding(new_doc)
            ...
            print(f"Embedded and updated doc: {new_doc.get('title', 'Unknown')}")
# Start watching
if __name__ == "__main__":
    while True:
        try:
            ...
            print("Error:", e)
            time.sleep(5)  # Retry after a pause